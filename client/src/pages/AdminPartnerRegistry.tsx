...
const partnerTypeOptions: Array<{
  value: AdminPartnerInput["partnerType"];
  label: string;
}> = [
...
function normalizePartnerType(
  value?: string
): AdminPartnerInput["partnerType"] {
  return partnerTypeOptions.some(option => option.value === value)
    ? (value as AdminPartnerInput["partnerType"])
    : "agency";
}
...
export default function AdminPartnerRegistry() {
  const [draft, setDraft] = useState<AdminPartnerInput>(() =>
    createDefaultPartnerDraft()
  );
  ...
  const updateDraft = <K extends keyof AdminPartnerInput>(
    key: K,
    value: AdminPartnerInput[K]
  ) => {
  ...
  async function submitPartner(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("error");
      setMessage(
        error instanceof Error ? error.message : "에이전트 저장에 실패했습니다."
      );
    }
  }
  ...
  async function removePartner(partner: BetaPartner) {
    ...
    if (
      !window.confirm(
        `${partner.name} 에이전트를 삭제할까요? 목록과 접근 권한에서 비활성화됩니다.`
      )
    )
      return;
    ...
    try {
      ...
      setStatus("error");
      setMessage(
        error instanceof Error ? error.message : "에이전트 삭제에 실패했습니다."
      );
    }
  }
          ...
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-base leading-7 text-ink-600">
                에이전시 기본 정보, 운영 이메일 인증, 지원 서비스, 선호 병원
                연결을 관리합니다.
              </p>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <Field label="유형">
                <select
                  ...
                  onChange={event =>
                    updateDraft(
                      "partnerType",
                      event.target.value as AdminPartnerInput["partnerType"]
                    )
                  }
              ...
              <Field label="수익 배분율(%)">
                <Input
                  ...
                  max={100}
                  step={0.1}
                  value={toPercent(draft.defaultRevenueShareRate)}
                  onChange={event =>
                    updateDraft(
                      "defaultRevenueShareRate",
              ...
              <OptionGroup
                ...
                onToggle={value =>
                  updateDraft("services", toggleValue(draft.services, value))
                }
              />
              ...
              <div className="md:col-span-2">
                ...
                <div className="grid max-h-64 gap-1.5 overflow-y-auto rounded-md border border-ink-200 bg-white p-2">
                  {providers.map(provider => (
                    <label
                      key={provider.id}
                      className="flex min-h-10 items-start gap-2 rounded-md px-2 py-1.5 text-sm hover:bg-ink-50"
                    >
                      <input
                        type="checkbox"
                        checked={draft.preferredProviderIds.includes(
                          provider.id
                        )}
                        onChange={() =>
                          updateDraft(
                            "preferredProviderIds",
                            toggleValue(draft.preferredProviderIds, provider.id)
                          )
                        }
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3">
                          ...
                          {partner.opsEmail || partner.contactEmail ? (
                            <div className="mt-1 text-xs text-ink-500">
                              {partner.opsEmail || partner.contactEmail}
                            </div>
                          ) : null}
                        ...
                        <td className="px-4 py-3 text-ink-600">
                          {partner.services.length
                            ...
                            : "-"}
                          <div className="mt-1 text-xs text-ink-500">
                            SLA {partner.slaHours}h
                          </div>
                        </td>
                        <td className="px-4 py-3 text-ink-600">
                          {partner.preferredProviderIds.length
                            ? partner.preferredProviderIds
                                .map(
                                  providerId =>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-2">
                            ...
                            <Button
                              ...
                              onClick={() => void removePartner(partner)}
                              disabled={isBusy}
                            >
