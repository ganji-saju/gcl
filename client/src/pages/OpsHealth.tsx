...
export default function OpsHealth() {
  ...
  const partnerRequestCount =
    meta?.partnerRequestCount ?? snapshot?.cases?.length ?? 0;
  const quoteRequestCount =
    meta?.quoteRequestCount ?? snapshot?.providerQuoteRequests?.length ?? 0;
  const quoteResponseCount =
    meta?.quoteResponseCount ?? snapshot?.quotes?.length ?? 0;
  ...
  const accountScopingReady = Boolean(
    meta?.authEmail && (meta?.scopedAccountEnabled || activeRole === "admin")
  );
  const notificationReady = Boolean(
    meta?.notificationDispatchConfigured || meta?.notificationOutboxConfigured
  );
  ...
  const partnerRequestTableReady = Boolean(
    meta?.partnerServiceRequestTableReady
  );
  const quoteRequestTableReady = Boolean(meta?.quoteRequestTableReady);
          ...
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-ink-600">
                관리자 API, Supabase v1 저장 파이프라인, 파트너/병원 운영
                데이터, 결제와 알림 설정을 한 곳에서 확인합니다.
              </p>
            </div>
            <Button
              type="button"
        ...
        <div className="container-wide grid gap-6 xl:grid-cols-[1fr_380px]">
          <div className="grid gap-6">
            ...
            <div className="grid gap-3 md:grid-cols-4">
              <MetricCard
                label="patients"
                value={storage?.patients ?? "-"}
                tone={countTone(storage?.patients)}
              />
              <MetricCard
                label="leads"
                value={storage?.leads ?? "-"}
                tone={countTone(storage?.leads)}
              />
              <MetricCard
                label="cases"
                value={storage?.cases ?? "-"}
                tone={countTone(storage?.cases)}
              />
              <MetricCard
                label="medical_intakes"
                value={storage?.medicalIntakes ?? "-"}
                tone={countTone(storage?.medicalIntakes)}
              />
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-4">
              <MetricCard
                label="admin routes"
                value={persistence?.adminLandingRoutes ?? "-"}
                tone={adminPersistenceReady ? "good" : "warn"}
              />
              <MetricCard
                label="contact channels"
                value={persistence?.contactChannels ?? "-"}
                tone={countTone(persistence?.contactChannels)}
              />
              <MetricCard
                label="provider profiles"
                value={persistence?.providerOperatingProfiles ?? "-"}
                tone={countTone(persistence?.providerOperatingProfiles)}
              />
              <MetricCard
                label="quality checks"
                value={persistence?.providerDataQualityChecks ?? "-"}
                tone={countTone(persistence?.providerDataQualityChecks)}
              />
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-4">
              <MetricCard
                label="파트너 서비스 요청"
                value={partnerRequestCount}
                tone={
                  partnerRequestCount > 0
                    ? "good"
                    : partnerRequestTableReady
                      ? "neutral"
                      : "warn"
                }
              />
              ...
              <MetricCard
                label="견적 응답"
                value={quoteResponseCount}
                tone={quoteResponseCount > 0 ? "good" : "neutral"}
              />
              <MetricCard
                label="현재 접근 역할"
                value={opsRoleLabel(activeRole)}
                tone={status === "ready" ? "good" : "neutral"}
              />
            </div>
            ...
            <div className="rounded-lg border border-ink-200 bg-white p-5">
              ...
              <CheckRow
                label="이메일 운영 인증"
                ok={status === "ready"}
                detail={`Supabase Auth 세션과 ops_user_access 허용 이메일을 함께 확인합니다. 현재 이메일: ${meta?.authEmail ?? "확인 전"}`}
              />
              <CheckRow
                label="Supabase v1 lead 저장 구조"
                ok={v1StorageReady}
                detail={`patients/leads/cases/medical_intakes row count를 확인합니다. 최신 lead: ${formatDate(storage?.latestLeadAt)}`}
              />
              <CheckRow
                label="Admin 데이터 persistence"
                ok={adminPersistenceReady}
                detail="admin_landing_routes, contact_channel_settings, provider_operating_profiles, provider_data_quality_checks, notification_outbox 테이블을 확인합니다."
              />
              <CheckRow
                label="파트너 서비스 요청 테이블"
                ok={partnerRequestTableReady}
                detail={`partner_service_requests 테이블 연결을 확인합니다. 상담 신청에서 파트너 지원 요청이 생성되면 현재 누적 ${partnerRequestCount}건이 증가합니다.`}
              />
              <CheckRow
                label="파트너/병원 기준 데이터"
                ok={Boolean(meta?.hasDbPartners && meta?.hasDbProviders)}
                detail="운영 파트너와 병원 후보 데이터가 Supabase에 등록되어 있어야 합니다."
              />
              <CheckRow
                label="견적 요청 흐름"
                ok={quoteRequestTableReady}
                detail={`quote_requests 테이블과 요청 생성 API를 확인합니다. 코디네이터가 병원 후보를 골라 견적 요청을 생성하면 현재 누적 ${quoteRequestCount}건이 증가합니다.`}
              />
              <CheckRow
                label="허용 이메일 테이블"
                ok={emailAccessReady}
                detail="ops_user_access에 관리자, 파트너, 병원 운영 이메일과 역할이 등록되어 있어야 합니다."
              />
              <CheckRow
                label="계정별 데이터 스코프"
                ok={accountScopingReady}
                detail="파트너/병원 역할은 ops_user_access의 partner_id 또는 provider_id 기준으로 데이터 범위를 제한합니다."
              />
              <CheckRow
                label="알림 발송/저장"
                ok={notificationReady}
                detail="notification_outbox 또는 외부 알림 gateway 설정을 확인합니다."
              />
              <CheckRow
                label="Stripe 예약금 결제"
                ok={stripeReady}
                detail={`Stripe Checkout Sessions API로 예약금 링크를 생성합니다. Vercel Production STRIPE_SECRET_KEY 설정 후 현재 모드가 test/live로 바뀝니다. 현재 모드: ${meta?.paymentMode ?? "확인 전"}`}
              />
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <div className="rounded-lg border border-ink-200 bg-white p-4">
                ...
                <div className="text-sm leading-6 text-ink-600">
                  관리자 전체 접근, 파트너 후보 선택, 병원 견적 제출 권한을
                  API와 화면 게이트에서 분리합니다.
                </div>
              </div>
              <div className="rounded-lg border border-ink-200 bg-white p-4">
                ...
                <div className="text-sm leading-6 text-ink-600">
                  견적 안내와 예약금 후속 메시지는 outbox에 저장하고, webhook이
                  있으면 외부 발송 gateway로 전달합니다.
                </div>
              </div>
              <div className="rounded-lg border border-ink-200 bg-white p-4">
                <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-ink-950">
                ...
                <div className="text-sm leading-6 text-ink-600">
                  Stripe Checkout 세션 생성 후 링크를 운영 화면에서 바로 열 수
                  있습니다.
                </div>
              </div>
            </div>
          ...
          <aside className="rounded-lg border border-ink-200 bg-white p-5">
            ...
            <div className="mt-5 rounded-md border border-coral-200 bg-coral-50 p-3 text-sm leading-6 text-coral-900">
              운영 전에는 허용 이메일, 공식 연락 채널 URL, Stripe Secret Key,
              알림 gateway 값을 Vercel 환경변수와 Supabase에 넣고 다시
              점검하세요.
            </div>
          </aside>
        </div>
