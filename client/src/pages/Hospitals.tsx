...
const LANGUAGES_FILTER: LanguageCode[] = [
  "en",
  "ko",
  "zh",
  "ja",
...
export default function Hospitals() {
  ...
  }, [hospitalSource, langFilter, region, search, specialty]);

  const toggleCompare = (hospital: Hospital, event: React.MouseEvent) => {
        ...
        <div className="container-wide">
          <div className="mb-5 flex flex-col gap-3 md:flex-row">
            ...
            <Button
              variant="outline"
              className={cn(
                "h-11 border-ink-200 text-ink-800",
                showFilters && "bg-teal-50 text-teal-800"
              )}
          ...
          <div className="mb-5 flex flex-wrap gap-2">
            {SPECIALTIES.map(item => (
              ...
              >
                {item === "all"
                  ? t("hospitals.filter.all")
                  : t(SPECIALTY_TRANSLATION_KEYS[item])}
              </button>
            ))}
          </div>
                      ...
                      <div className="card-hover h-full overflow-hidden rounded-lg border border-ink-200 bg-white hover:border-teal-300">
                        ...
                        <div className="p-5">
                          <div className="mb-3 flex items-center justify-between gap-2">
                            ...
                            >
                              {t(
                                SPECIALTY_TRANSLATION_KEYS[hospital.specialty]
                              )}
                            </span>
                          ...
                          <div className="mt-4 grid gap-2 rounded-md border border-ink-200 bg-ink-50 p-3 text-xs text-ink-700">
                            ...
                            <div className="grid gap-2 sm:grid-cols-2">
                              <span className="flex items-center gap-1.5">
                                ...
                                {t("hospitals.slaUnder")}{" "}
                                {hospital.responseSlaHours}
                                {t("hospitals.hours")}
                              </span>
                              <span className="flex items-center gap-1.5">
                                <WalletCards className="size-3.5 text-teal-700" />
                                {formatUSD(hospital.packagePriceMinUsd * 1300)}{" "}
                                -{" "}
                                {formatUSD(hospital.packagePriceMaxUsd * 1300)}
                              </span>
                            </div>
                          </div>
                          <div className="mt-4 flex flex-wrap gap-1.5">
                            {hospital.specialties
                              ...
                              .map(specialtyItem => (
                                <span
                                  key={specialtyItem}
                                  className="rounded bg-ink-50 px-2 py-1 text-xs text-ink-600"
                                >
                                  {specialtyItem}
                                </span>
                              ))}
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        inCompare
                          ...
                          : "bg-white text-ink-800 hover:bg-ink-950 hover:text-white"
                      )}
                    >
                      {inCompare ? (
